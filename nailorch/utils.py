...
import numpy as np
from nailorch import as_variable
from nailorch import Variable
...
def get_dot_graph(output, verbose=True):
    ...
    Args:
        output (nailorch.Variable): Output variable from which the graph is
            constructed.
...
def plot_dot_graph(output, verbose=True, to_file='graph.png'):
    ...
    tmp_dir = os.path.join(os.path.expanduser('~'), '.nailorch')
...
def reshape_sum_backward(gy, x_shape, axis, keepdims):
    """Reshape gradient appropriately for nailorch.functions.sum's backward.

    Args:
        gy (nailorch.Variable): Gradient variable from the output by backprop.
    ...
    Returns:
        nailorch.Variable: Gradient variable which is reshaped appropriately
    """
    ndim = len(x_shape)
    tupled_axis = axis
    if axis is None:
...
def gradient_check(f, x, *args, rtol=1e-4, atol=1e-5, **kwargs):
    ...
    Args:
        ...
        x (`ndarray` or `nailorch.Variable`): A traget `Variable` for computing
            the gradient.
...
def numerical_grad(f, x, *args, **kwargs):
    ...
    Args:
        f (callable): A function which gets `Variable`s and returns `Variable`s.
        x (`ndarray` or `nailorch.Variable`): A target `Variable` for computing
            the gradient.
        *args: If `f` needs variables except `x`, you can specify with this
            argument.
        **kwargs: If `f` needs keyword variables, you can specify with this
...
def array_equal(a, b):
    ...
    Args:
        a, b (numpy.ndarray or cupy.ndarray or nailorch.Variable): input arrays
            to compare
...
cache_dir = os.path.join(os.path.expanduser('~'), '.nailorch')


def get_file(url, file_name=None):
    """Download a file from the `url` if it is not in the cache.

    The file at the `url` is downloaded to the `~/.nailorch`.
